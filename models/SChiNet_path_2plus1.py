...
class SChiStage(nn.Module):
	def __init__(self, in_channels1, out_channels1, in_channels2, out_channels2, heads=1,
		...
		self.pw1_a = nn.Sequential(
			CBlock(in_channels1, out_channels1, heads,
				conv=MultiHeadConv3d(*((in_channels1*heads, out_channels1*heads, 1, in_channels1*heads) if fullconv else (in_channels1, out_channels1, heads, in_channels1)),
				       shared_map=shared_map, kernel_size=(1, kernel_size[1], kernel_size[2]), stride=(1, stride1[1], stride1[2]), padding='same'),
				proj=Column(MultiHeadResBlock(out_channels1, heads,
					conv=MultiHeadConv3d(out_channels1, out_channels1, heads, out_channels1, shared_map=shared_map,
						   kernel_size=(1, res_kernel_size[1], res_kernel_size[2]), stride=1, padding='same'),
					act=act, norm=norm, order='CANCANS'), depth=depth, recurrent=False),
	            act=act, norm=norm, order='CANPN'),
			CBlock(out_channels1, out_channels1, heads,
				conv=MultiHeadConv3d(*((out_channels1*heads, out_channels1*heads, 1, out_channels1*heads) if fullconv else (out_channels1, out_channels1, heads, out_channels1)),
				       shared_map=shared_map, kernel_size=(kernel_size[0], 1, 1), stride=(stride1[0], 1, 1), padding='same'),
				proj=Column(MultiHeadResBlock(out_channels1, heads,
					conv=MultiHeadConv3d(out_channels1, out_channels1, heads, out_channels1, shared_map=shared_map,
						   kernel_size=(res_kernel_size[0], 1, 1), stride=1, padding='same'),
					act=act, norm=norm, order='CANCANS'), depth=depth, recurrent=False),
	            act=act, norm=norm, order='CANPN')
		)

		self.pw1_b = nn.Sequential(
			CBlock(in_channels_b, out_channels_b, heads,
				conv=MultiHeadConv3d(*((in_channels_b*heads, out_channels_b*heads, 1, in_channels_b*heads) if fullconv else (in_channels_b, out_channels_b, heads, in_channels_b)),
				       shared_map=shared_map, kernel_size=(1, kernel_size_b[1], kernel_size_b[2]), stride=(1, sp[1], sp[2]), padding='same'),
				proj=Column(MultiHeadResBlock(out_channels_b, heads,
					conv=MultiHeadConv3d(out_channels_b, out_channels_b, heads, out_channels_b, shared_map=shared_map,
						   kernel_size=(1, res_kernel_size_b[1], res_kernel_size_b[2]), stride=1, padding='same'),
					act=act, norm=norm, order='CANCANS'), depth=depth, recurrent=False),
	            act=act, norm=norm, order='CANPN'),
			CBlock(out_channels_b, out_channels_b, heads,
				conv=MultiHeadConv3d(*((out_channels_b*heads, out_channels_b*heads, 1, out_channels_b*heads) if fullconv else (out_channels_b, out_channels_b, heads, out_channels_b)),
				       shared_map=shared_map, kernel_size=(kernel_size_b[0], 1, 1), stride=(sp[0], 1, 1), padding='same'),
				proj=Column(MultiHeadResBlock(out_channels_b, heads,
					conv=MultiHeadConv3d(out_channels_b, out_channels_b, heads, out_channels_b, shared_map=shared_map,
						   kernel_size=(res_kernel_size_b[0], 1, 1), stride=1, padding='same'),
					act=act, norm=norm, order='CANCANS'), depth=depth, recurrent=False),
	            act=act, norm=norm, order='CANPN')
		)

		self.pw2_a = nn.Sequential(
			CBlock(in_channels2, out_channels2, heads,
				conv=MultiHeadConv3d(*((in_channels2*heads, out_channels2*heads, 1, in_channels2*heads) if fullconv else (in_channels2, out_channels2, heads, in_channels2)),
				       shared_map=shared_map, kernel_size=(1, kernel_size[1], kernel_size[2]), stride=(1, stride2[1], stride2[2]), padding='same'),
				proj=Column(MultiHeadResBlock(out_channels2, heads,
					...
					act=act, norm=norm, order='CANCANS'), depth=depth, recurrent=False),
	            act=act, norm=norm, order='CANPN'),
			CBlock(out_channels2, out_channels2, heads,
				conv=MultiHeadConv3d(*((out_channels2*heads, out_channels2*heads, 1, out_channels2*heads) if fullconv else (out_channels2, out_channels2, heads, out_channels2)),
				       shared_map=shared_map, kernel_size=(kernel_size[0], 1, 1), stride=(stride2[0], 1, 1), padding='same'),
				proj=Column(MultiHeadResBlock(out_channels2, heads,
					conv=MultiHeadConv3d(out_channels2, out_channels2, heads, out_channels2, shared_map=shared_map,
						   kernel_size=(res_kernel_size[0], 1, 1), stride=1, padding='same'),
					act=act, norm=norm, order='CANCANS'), depth=depth, recurrent=False),
