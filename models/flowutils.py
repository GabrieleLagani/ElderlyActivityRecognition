...
class Centered_Grad(nn.Module):
    ...
    def __init__(self):
        ...
        self.x_ker_init = torch.tensor([[[[-0.5, 0, 0.5]]]],dtype=torch.get_default_dtype(), requires_grad=True)
        self.y_ker_init = torch.tensor([[[[-0.5], [0], [0.5]]]],dtype=torch.get_default_dtype(), requires_grad=True)
...
class Forward_Grad(nn.Module):
    ...
    def __init__(self):
        ...
        # initialize center kernels
        self.x_ker_init = torch.tensor([[[[-1, 1]]]],dtype=torch.get_default_dtype(), requires_grad=True)
        self.y_ker_init = torch.tensor([[[[-1], [1]]]],dtype=torch.get_default_dtype(), requires_grad=True)
...
class IterBlock(nn.Module):
    ...
    def forward(self, diff2_x_warp, diff2_y_warp, u1, u2, grad, p11 , p12, p21, p22,
        ...
        p11 = (p11 + self.taut * u1x) / (1.0 + self.taut * torch.sqrt(u1x.pow(2) + u1y.pow(2) + self.GRAD_IS_ZERO))
        p12 = (p12 + self.taut * u1y) / (1.0 + self.taut * torch.sqrt(u1x.pow(2) + u1y.pow(2) + self.GRAD_IS_ZERO))
        p21 = (p21 + self.taut * u2x) / (1.0 + self.taut * torch.sqrt(u2x.pow(2) + u2y.pow(2) + self.GRAD_IS_ZERO))
        p22 = (p22 + self.taut * u2y) / (1.0 + self.taut * torch.sqrt(u2x.pow(2) + u2y.pow(2) + self.GRAD_IS_ZERO))
