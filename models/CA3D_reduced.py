...
def make_attn_layer(config, channels, heads, headsize, token_dim=None, transposed=False):
	...
	if attn_type == 'LocalAttention':
		qkv_splits, *qkv_params = get_proj_params(channels, LocalAttention.QKV_SPLITS, heads, headsize, transposed, shared_qkv)
...
def make_proj_layer(config, in_channels, out_channels, heads, headsize, token_dim=None, transposed=False, splits=None):
	...
	headwise_map = config.get('headwise_map', True)
	shared_map = config.get('shared_map', False)
	proj_kernel_size = config.get('proj_kernel_size', 1)
	...
	if not headwise_map:
		in_channels = in_channels * heads
		out_channels = out_channels * heads
		headsize = headsize * heads
		heads = 1
